  ...
  <view class="ui-box">
    <view style="padding: 20rpx 30rpx">
      <view class="box-title">
        ...
        <view class="event-status">
          <text class="urgent common" v-if="msgDetail.eventLevel == 'urgent'"
            >紧急事件</text
          >
          <text class="nourgent common" v-else>一般事件</text>
          <text class="nodeal common" v-if="msgDetail.operateFlag == '0'"
            >未处理</text
          >
      ...
      <view class="content">
        ...
        >
          <rich-text
            :nodes="$u.trim(msgDetail.content, 'all') || '--'"
          ></rich-text>
        </u-read-more>
    ...
    <view class="action">
      <template v-if="msgDetail.eventLevel == 'urgent'">
        <view
          class="detail"
          v-if="msgDetail.operateFlag == '0'"
          @click="handleRead(msgDetail.msgId)"
          >我来处理
        </view>
        <view class="detail disabled" v-else>我已处理</view>
        <view
          class="phone"
          @click="handleCall(msgDetail.phone)"
          v-if="msgDetail.operateFlag == '0'"
        >
          <u-icon name="phone" color="white" size="22"></u-icon
          ><text style="margin-left: 8rpx">他人处理</text>
        </view>
