				...
				<swiper-item class="picker-calendar" v-for="(calendar,calendarIndex2) in calendars"
					...
					<view class="picker-calendar-view" v-for="(date,dateIndex) in calendar" :key="dateIndex"
						...
						<view class="picker-calendar-view-dot"
							:style="{opacity: date.dotStyle.opacity, background: getLightDot(date)}"></view>
						<!-- 	<view class="picker-calendar-view-dot" :style="getLightDot(date)"></view> -->
						<!-- 信息样式 -->
	...
	export default {
		...
		methods: {
			...
			onSetYear(value) {
				...
				this.$emit('monthChange', this.date);
			...
			//处理日历
			procCalendar(item) {
				// item.dateFormat = uni.$u.timeFormat(item.dateObj, 'yyyy-mm-dd')
				//定义初始样式
				item.statusStyle = {
					opacity: 1,
				...
				// }
				// if (item.statusStyle.background != this.color) {
				// 	if (this.lightDot.includes(item.dateFormat)) {
				// 		item.dotStyle.background = this.color
				// 	}
				// } else {
				// 	item.title = item.dateObj.getDate();
		...
		computed: {
			...
			getLightDot() {
				return function(item) {
					const dateFormat = uni.$u.timeFormat(item.dateObj, 'yyyy-mm-dd')
					return this.lightDot.includes(dateFormat) ? this.color : 'transparent'
				}
			}
		},
		watch: {
