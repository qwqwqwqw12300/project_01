		...
		<app-body>
				...
				<u-grid col="2">
					<u-grid-item v-for="(baseListItem, baseListIndex) in baseList" :key="baseListIndex">
						<view class="ui-menu-item" :border="false">
							<u-icon :customStyle="{ paddingTop: 20 + 'rpx' }" name="/static/images/myself/device.png" size="80rpx"></u-icon>
							<!-- <text class="grid-text">{{ baseListItem.title }}</text> -->
							<u-text class="grid-text" @click="edit" suffixIcon="edit-pen-fill" iconStyle="font-size: 36rpx" align="center" :text="baseListItem.title"></u-text>
							<text class="grid-text">{{ baseListItem.site }}</text>
							<view class="ui-wifi active"><u-icon :customStyle="{ paddingTop: 20 + 'rpx' }" name="wifi" size="40rpx" color="#0dab1c"></u-icon></view>
							<u-icon @click="onDelete" class="ui-close active" name="close-circle-fill" size="40rpx"></u-icon>
						</view>
					...
					<u-grid-item v-for="(baseListItem, baseListIndex) in baseList" :key="baseListIndex + 1">
						<view class="ui-menu-item" :border="false">
							...
							<u-text class="grid-text" @click="edit" suffixIcon="edit-pen-fill" iconStyle="font-size: 36rpx" align="center" :text="baseListItem.title"></u-text>
							<text class="grid-text">{{ baseListItem.site }}</text>
							<view class="ui-wifi"><u-icon :customStyle="{ paddingTop: 20 + 'rpx' }" name="wifi-off" size="40rpx" color="#ff4800"></u-icon></view>
							<u-icon @click="onDelete" class="ui-close active" name="close-circle-fill" size="40rpx"></u-icon>
						</view>
			...
			<u-popup :closeable="true" :round="10" :show="bindRoomShow" mode="center" @close="close">
				<view class="wd-add">
					...
					<view class="ui-add-box">
						...
						<view class="ui-select"><uni-data-select v-model="value" :clear="false" :localdata="range"></uni-data-select></view>
					</view>
					<view class="ui-add-box">
						<u-text size="28rpx" prefixIcon="home-fill" iconStyle="font-size: 36rpx" text="选择房间"></u-text>
						<view class="ui-select"><uni-data-select v-model="value" :clear="false" :localdata="range"></uni-data-select></view>
					</view>
					<view class="wd-btn-gloup"><button @click="close" type="default">确定</button></view>
				</view>
			</u-popup>
			<!-- /绑定房间 -->
			<!-- 修改名称 -->
			<u-popup :closeable="true" :round="10" :show="isEditShow" mode="center" @close="eidtClose">
				<view class="wd-add ui-change">
					<u-text prefixIcon="edit-pen" :iconStyle="{ fontSize: '38rpx', color: '#ea942f' }" color="#ea942f" size="30rpx" text="修改名称"></u-text>
					<view class="ui-add-box">
					...
					<view class="wd-btn-gloup"><button @click="eidtClose" type="default">确定</button></view>
				</view>
			</u-popup>
			<!-- /修改名称 -->
		</app-body>
...
export default {
	data() {
		return {
			...
			bindRoomShow: false,
			value: 0,
			range: [{ value: 0, text: '房间一' }, { value: 1, text: '房间二' }, { value: 2, text: '房间三' }],
			baseList: [
				{
					title: '家庭1',
					site: '卧室',
