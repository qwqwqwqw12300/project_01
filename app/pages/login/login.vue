	...
	<view class="wd-bg ui-body">
		...
		<view class="ui-nav">
			<u-tabs
				lineWidth="160rpx"
				lineColor="#fdbd2d"
				:itemStyle="{ width: '200rpx', height: '90rpx' }"
				:activeStyle="{ color: '#fdbd2d', fontSize: '28rpx' }"
				:inactiveStyle="{ color: '#bfbfbf', fontSize: '28rpx' }"
				lineHeight="4.5rpx"
				:list="navList"
				@click="navClick"
			></u-tabs>
		...
		<u-transition mode="fade-left" :show="navActive === 0 && delay === 0">
			<view class="ui-form">
				<view class="ui-form-item">
					<u-text prefixIcon="phone-fill" iconStyle="font-size: 30rpx" text="手机号码" color="#444" size="28rpx"></u-text>
				...
				<view class="ui-bot">
					...
					<text class="active">忘记密码？</text>
				</view>
				<view class="ui-btn"><button type="default" @click="login">立即登录</button></view>
			</view>
		</u-transition>
		<!-- /账号登录 -->
		<!-- 验证码登录 -->
		<u-transition  mode="fade-right" :show="navActive !== 0 && delay !== 0">
			<view class="ui-form">
				...
				<view class="ui-form-item">
					<u-text prefixIcon="email-fill" iconStyle="font-size: 32rpx" text="验证码" color="#444" size="28rpx"></u-text>
					<view class="ui-input">
						<u-input placeholder="请输入短信认证码" :border="'none'" fontSize="28rpx" clearable>
							<template slot="suffix">
								<button type="default" class="wd-sms" size="mini">验证码</button>
							</template>
						</u-input>
					</view>
				</view>
