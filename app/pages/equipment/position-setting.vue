		...
		<view id="setting">
			...
			<view class="ui-add">
				<view>
					...
					<view class="ui-input"><u-input v-model="zoneInfo.name" placeholder="请输入子区域名称" :border="'none'" fontSize="28rpx" clearable></u-input></view>
				</view>
				<view>
					...
					<view class="ui-shape">
						<view>
							<text>长</text>
							<view class="ui-input">
								<u-input type="number" v-model="zoneInfo.height" placeholder="请输入长度" :border="'none'" fontSize="28rpx">
						...
						<view>
							<text>宽</text>
							<view class="ui-input">
								<u-input type="number" v-model="zoneInfo.width" placeholder="请输入宽度" :border="'none'" fontSize="28rpx">
									<template slot="suffix">
										<text>米</text>
									</template>
								</u-input>
...
export default {
	...
	methods: {
		 ...
		 */
		addZone() {
			const { name, width, height } = this.zoneInfo;
		...
		radarDevice() {
			...
			const list = this.roomZones.map(ele => {
				const { old, width, height, startTime, endTime, entryTime, departureTime } = ele,
					obj = assignDeep(
						{},
						{
							...ele,
		...
		touchstart(event, item, index) {
			const { clientX, clientY } = event.touches[0],
				{ offsetLeft, offsetTop } = this.touchInfo;
				...
				for (let j = 0; j <= Math.abs(countX); j++) {
					...
					if (countX >= 0) { // 正向移动
						if (currentLine === 0 || index + 1 <= (currentLine === 0 && countY >= 0 ) ? line : currentLine * line) {
							...
							console.log('跳出', index + 1, (currentLine === 0 && countY >= 0 ) ? line : currentLine * line);
							break;
						}
					} else { // 反向移动
						if(currentCol === 0) {
